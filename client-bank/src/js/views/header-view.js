...
const createNavMenuTemplate = (item, currentRoute) => {
  const isActive = item.route === currentRoute ? true : false;
...
const NavRender = (isAuth, currentRoute) => {
  if (isAuth) {
    return `<nav class="header__nav">
      <ul class="header__list">
      ${navListArr
        .map((item) => {
          return createNavMenuTemplate(item, currentRoute);
        })
        .join("")}
      </ul>
    </nav>`;
...
const BugerRender = (isAuth) => {
  if (isAuth) {
    return `<button class="burger js-burger">
...
const createHeaderTemplate = (currentRoute, isAuth) => {
  return `<div class="container header__container">
    ...
    ${NavRender(isAuth, currentRoute)}
  </div>`;
};

export default class HeaderView extends AbstractView {
  constructor(currentRoute, isAuth) {
    super();
  ...
  getHeaderContainer() {
    return this.getElement().querySelector(`.header__container`);
  }

  getTemplate() {
    return createHeaderTemplate(this._currentRoute, this._isAuth);
  }
}
